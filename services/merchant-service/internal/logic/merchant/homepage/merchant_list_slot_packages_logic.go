...
func NewMerchantListSlotPackagesLogic(svcCtx *svc.ServiceContext) *MerchantListSlotPackagesLogic {
...
func (l *MerchantListSlotPackagesLogic) MerchantListSlotPackages(ctx context.Context, req *types.PageReq) (resp *types.PageListResp, err error) {
	_ = fmt.Sprintf
	_ = url.Values{}
raw, err := httpinvoke.Run(ctx, "GET", "/api/v1/merchant/homepage-packages", nil, url.Values{"page": {fmt.Sprintf("%d", req.Page)}, "page_size": {fmt.Sprintf("%d", req.PageSize)}}, nil, hmerchant.NewHomepageSlotHandler(l.svcCtx).MerchantListSlotPackages)
	if err != nil {
		return nil, err
	}
