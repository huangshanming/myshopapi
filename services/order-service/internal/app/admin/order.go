...
func (h *OrderHandler) AdminList(w http.ResponseWriter, r *http.Request) {
	...
	orders, total, err := h.logic.ListAll(r.Context(), shopID, p, ps, r.URL.Query().Get("status"), r.URL.Query().Get("order_no"))
	if err != nil {
...
func (h *OrderHandler) AdminDetail(w http.ResponseWriter, r *http.Request) {
	...
	order, err := h.logic.GetOrderAdmin(r.Context(), orderID)
	if err != nil {
