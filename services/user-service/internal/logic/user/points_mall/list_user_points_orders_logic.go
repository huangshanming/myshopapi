...
func (l *ListUserPointsOrdersLogic) ListUserPointsOrders(ctx context.Context, req *types.PageReq) (resp *types.PageListResp, err error) {
	data, err := huser.NewPointsOrderHandler(l.svcCtx).List(ctx, appinput.CallInput{Query: url.Values{"page": {fmt.Sprintf("%d", req.Page)}, "page_size": {fmt.Sprintf("%d", req.PageSize)}}})
	if err != nil {
		return nil, err
	}
