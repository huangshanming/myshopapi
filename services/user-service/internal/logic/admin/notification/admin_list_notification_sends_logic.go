...
func (l *AdminListNotificationSendsLogic) AdminListNotificationSends(ctx context.Context, req *types.PageReq) (resp *types.PageListResp, err error) {
	data, err := hadmin.NewAdminHandler(l.svcCtx).AdminListNotificationSends(ctx, appinput.CallInput{Query: url.Values{"page": {fmt.Sprintf("%d", req.Page)}, "page_size": {fmt.Sprintf("%d", req.PageSize)}}})
	if err != nil {
		return nil, err
	}
