...
func (l *PlatformProductLogic) List(ctx context.Context, f repository.ProductListFilter) (map[string]interface{}, error) {
...
func (l *PlatformProductLogic) ForceOffSale(ctx context.Context, id, operatorID uint64, remark string) error {
	remark = strings.TrimSpace(remark)
	if remark == "" {
		return errors.New("请填写备注，将通知商家")
	}
	p, err := l.svcCtx.ProductAdmin.GetByID(ctx, id)
	if err != nil {
	...
	if err := l.svcCtx.ProductAdmin.SetStatus(ctx, id, p.ShopID, model.ProductOffSale); err != nil {
		return err
	}
	after, _ := json.Marshal(map[string]string{"status": model.ProductOffSale, "remark": remark})
	_ = l.svcCtx.ProductAdmin.AddOpLog(ctx, p.ShopID, &id, operatorID, "platform_off_sale", "", string(after))
...
func (l *PlatformProductLogic) SoftDelete(ctx context.Context, id, operatorID uint64, remark string) error {
	remark = strings.TrimSpace(remark)
	if remark == "" {
		return errors.New("请填写备注，将通知商家")
	}
	...
	if err := l.svcCtx.ProductAdmin.SetStatus(ctx, id, p.ShopID, model.ProductDeleted); err != nil {
		return err
	}
	after, _ := json.Marshal(map[string]string{"status": model.ProductDeleted, "remark": remark})
	_ = l.svcCtx.ProductAdmin.AddOpLog(ctx, p.ShopID, &id, operatorID, "platform_delete", "", string(after))
	_ = l.svcCtx.Notifications.Create(ctx, &notifymodel.ShopNotification{
		ShopID:  p.ShopID,
