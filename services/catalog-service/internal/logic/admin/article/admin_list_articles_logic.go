...
func NewAdminListArticlesLogic(ctx context.Context, svcCtx *svc.ServiceContext) *AdminListArticlesLogic {
	return &AdminListArticlesLogic{Logger: logx.WithContext(ctx), svcCtx: svcCtx}
}

func (l *AdminListArticlesLogic) AdminListArticles(ctx context.Context, req *types.AdminArticleListReq) (resp *types.PageListResp, err error) {
	f := repository.ArticleListFilter{
		Title: req.Title, AuditStatus: req.AuditStatus, Status: req.Status,
		Page: req.Page, PageSize: req.PageSize, Recycle: false,
	}
	if req.ShopId > 0 {
		f.ShopID = req.ShopId
		f.FilterShop = true
