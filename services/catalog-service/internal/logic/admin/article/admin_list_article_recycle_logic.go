...
func NewAdminListArticleRecycleLogic(svcCtx *svc.ServiceContext) *AdminListArticleRecycleLogic {
...
func (l *AdminListArticleRecycleLogic) AdminListArticleRecycle(ctx context.Context, req *types.PageReq) (resp *types.PageListResp, err error) {
	_ = fmt.Sprintf
	_ = url.Values{}
	raw, err := httpinvoke.Run(ctx, "GET", "/api/v1/admin/articles/recycle", nil, url.Values{"page": {fmt.Sprintf("%d", req.Page)}, "page_size": {fmt.Sprintf("%d", req.PageSize)}}, nil, hadmin.NewArticleHandler(l.svcCtx).List)
	if err != nil {
		return nil, err
	}
