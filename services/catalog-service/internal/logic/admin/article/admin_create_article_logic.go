...
func NewAdminCreateArticleLogic(svcCtx *svc.ServiceContext) *AdminCreateArticleLogic {
...
func (l *AdminCreateArticleLogic) AdminCreateArticle(ctx context.Context, req *types.JSONBody) (resp *types.AnyResp, err error) {
	_ = fmt.Sprintf
	_ = url.Values{}
	raw, err := httpinvoke.Run(ctx, "POST", "/api/v1/admin/articles", nil, nil, req, hadmin.NewArticleHandler(l.svcCtx).Create)
	if err != nil {
		return nil, err
	}
	var data interface{}
