...
func NewAdminListProductsLogic(svcCtx *svc.ServiceContext) *AdminListProductsLogic {
...
func (l *AdminListProductsLogic) AdminListProducts(ctx context.Context, req *types.PageReq) (resp *types.PageListResp, err error) {
	_ = fmt.Sprintf
	_ = url.Values{}
	raw, err := httpinvoke.Run(ctx, "GET", "/api/v1/admin/products", nil, url.Values{"page": {fmt.Sprintf("%d", req.Page)}, "page_size": {fmt.Sprintf("%d", req.PageSize)}}, nil, hadmin.NewPlatformProductHandler(l.svcCtx).List)
	if err != nil {
		return nil, err
	}
