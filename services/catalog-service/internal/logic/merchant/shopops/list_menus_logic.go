...
func NewListMenusLogic(svcCtx *svc.ServiceContext) *ListMenusLogic {
...
func (l *ListMenusLogic) ListMenus(ctx context.Context, req *types.PageReq) (resp *types.PageListResp, err error) {
	_ = fmt.Sprintf
	_ = url.Values{}
	raw, err := httpinvoke.Run(ctx, "GET", "/api/v1/merchant/shop/menus", nil, url.Values{"page": {fmt.Sprintf("%d", req.Page)}, "page_size": {fmt.Sprintf("%d", req.PageSize)}}, nil, hhandler.NewShopOpsHandler(l.svcCtx).ListMenus)
	if err != nil {
		return nil, err
	}
