...
func NewMerchantGetProductLogic(svcCtx *svc.ServiceContext) *MerchantGetProductLogic {
...
func (l *MerchantGetProductLogic) MerchantGetProduct(ctx context.Context, req *types.IdPathReq) (resp *types.AnyResp, err error) {
	_ = fmt.Sprintf
	_ = url.Values{}
	raw, err := httpinvoke.Run(ctx, "GET", "/api/v1/merchant/products/:id", map[string]string{"id": fmt.Sprintf("%d", req.Id)}, nil, nil, hmerchant.NewProductHandler(l.svcCtx).Detail)
	if err != nil {
		return nil, err
	}
	var data interface{}
