...
func NewDeleteAttrTemplateLogic(svcCtx *svc.ServiceContext) *DeleteAttrTemplateLogic {
...
func (l *DeleteAttrTemplateLogic) DeleteAttrTemplate(ctx context.Context, req *types.IdPathReq) (resp *types.AnyResp, err error) {
	_ = fmt.Sprintf
	_ = url.Values{}
	raw, err := httpinvoke.Run(ctx, "DELETE", "/api/v1/merchant/attr-templates/:id", map[string]string{"id": fmt.Sprintf("%d", req.Id)}, nil, nil, hmerchant.NewProductHandler(l.svcCtx).DeleteAttrTemplate)
	if err != nil {
		return nil, err
	}
	var data interface{}
