...
func NewMerchantListArticleCommentsLogic(svcCtx *svc.ServiceContext) *MerchantListArticleCommentsLogic {
...
func (l *MerchantListArticleCommentsLogic) MerchantListArticleComments(ctx context.Context, req *types.PageReq) (resp *types.PageListResp, err error) {
	_ = fmt.Sprintf
	_ = url.Values{}
	raw, err := httpinvoke.Run(ctx, "GET", "/api/v1/merchant/article-comments", nil, url.Values{"page": {fmt.Sprintf("%d", req.Page)}, "page_size": {fmt.Sprintf("%d", req.PageSize)}}, nil, hmerchant.NewArticleHandler(l.svcCtx).CommentList)
	if err != nil {
		return nil, err
	}
