-- goctl model DDL for catalog-service (extracted; authority remains scripts/*.sql)
...
CREATE TABLE IF NOT EXISTS community_article (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    author_user_id BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT ''C端作者用户ID,
    audience_count BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '去重观众数',
    IF NOT EXISTS comment_count INT NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS community_article_comment (
  ...
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    IF NOT EXISTS parent_id BIGINT UNSIGNED NOT NULL DEFAULT 0 COMMENT '父评论ID，0=一级',
  KEY idx_article (article_id),
  KEY idx_shop (shop_id),
    PRIMARY KEY (id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;;
