      ...
      <el-table-column label="操作" width="220" fixed="right">
        <template #default="{ row }">
          <template v-if="row.status === 'pending'">
            <el-button v-permission="'aftersale:handle'" link type="success" @click="doHandle(row, 'approve')">同意</el-button>
            <el-button v-permission="'aftersale:handle'" link type="danger" @click="doHandle(row, 'reject')">拒绝</el-button>
          </template>
          <el-button
            v-if="row.status === 'pending' || row.status === 'approved'"
            v-permission="'aftersale:handle'"
            link
            type="warning"
